...
def plot_predictions(y_test, y_pred, dataset_name):
    ...
    plt.title(f'Ensemble Model Actual vs Predicted {dataset_name} Grades')
...
feature_importances_math_ensemble = None
feature_importances_por_ensemble = None
...
for dataset_name, X_train, X_test, y_train, y_test in [
    ...
    y_pred_ensemble = (y_pred_lr + y_pred_rf + y_pred_gb) / 3
    
    # Evaluate ensemble model
    mse, rmse, mae, r2 = evaluate_model(y_test, y_pred_ensemble)
    print(f"{dataset_name} Ensemble Model - MSE: {mse}, RMSE: {rmse}, MAE: {mae}, R²: {r2}")
    
    # Cross-validation
    mse_cv, rmse_cv, mae_cv, r2_cv = cross_validate_model(model_rf, X_train, y_train)
    ...
    if dataset_name == 'Math':
        feature_importances_math_ensemble = (model_rf.feature_importances_ + model_gb.feature_importances_) / 2
    else:
        feature_importances_por_ensemble = (model_rf.feature_importances_ + model_gb.feature_importances_) / 2
    
    # Plot Actual vs Predicted values
    plt.figure(figsize=(10, 5))
    plot_predictions(y_test, y_pred_ensemble, dataset_name)
    plt.show()
...
plt.bar(index, feature_importances_math_ensemble, bar_width, label='Math')
plt.bar(index + bar_width, feature_importances_por_ensemble, bar_width, label='Por')

plt.xlabel('Features')
plt.ylabel('Feature Importance')
plt.title('Ensemble Model Feature Importances for Math and Por Datasets')
