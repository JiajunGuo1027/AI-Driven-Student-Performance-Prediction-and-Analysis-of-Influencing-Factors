...
def plot_predictions(y_test, y_pred, dataset_name):
    ...
    plt.title(f'Linear Regression Actual vs Predicted {dataset_name} Grades')
...
for dataset_name, X_train, X_test, y_train, y_test in [
    ...
    if dataset_name == 'Math':
        coeff_math = model.coef_
    else:
        coeff_por = model.coef_
...
plt.bar(index, coeff_math, bar_width, label='Math')
plt.bar(index + bar_width, coeff_por, bar_width, label='Por')

plt.xlabel('Features')
plt.ylabel('Coefficient Value')
plt.title('Linear Regression Feature Importances for Math and Por Datasets')
